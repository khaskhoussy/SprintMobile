...
public class ajouter_compteController extends Controller {
    ...
    public String uploadPhoto(String path) {
        ...
        try {
            ...
            cr.setUrl(Controller.ip + "/ScriptPHP/Utilisateur/uploadPhoto.php");
            cr.setPost(true);
    ...
    public void add_user(String username, String password, String nom, String email, String prenom, String imgUpPath,String phone) {
        //String dbName = uploadPhoto(imgUpPath);
        String dbName = uploadPhoto(imgUpPath);
        String hashed = BCrypt.hashpw(password, BCrypt.gensalt(13));
        System.err.println(hashed);

        ConnectionRequest con = new ConnectionRequest(Controller.ip + "/ScriptPHP/Utilisateur/insert.php",true);
        con.addArgument("password", hashed.substring(0, 2) + "y" + hashed.substring(3));
        con.addArgument("roles", "a:1:{i:0;s:11:\"ROLE_CLIENT\";}");
        con.addArgument("image", dbName);
        con.addArgument("nom", nom);
        con.addArgument("prenom", prenom);
        con.addArgument("email", email);
        con.addArgument("username", username);
        con.addArgument("telephone", phone);
